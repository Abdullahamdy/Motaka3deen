            ...
            @if(!empty($records) && count($records)>0)
                <div class="table-responsive">
                    <table class="table table-bordered">
                        ...
                        <th class="text-center"> نص السؤال </th>
                        ...
                        @foreach($records as $record)
                                ...
                                <td>{{optional($record)->question}}</td>
                                <td>{{optional($record)->answer}}</td>




                                 <td class="text-center"><a href="{{url('admin/question/' . $record->id .'/edit')}}" class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a></td>
                                 <td class="text-center">
                                    <button
                                    id="{{$record->id}}"
                                    data-token="{{ csrf_token() }}"
                                    data-route="{{url('admin/question/'.$record->id)}}"
                                    type="button"
